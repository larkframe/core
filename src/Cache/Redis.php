...
class Redis
{
    ...
    /**
     * Redis client type constants.
     */
    public const PHPREDIS_CLIENT = 'phpredis';
    public const PREDIS_CLIENT = 'predis';

    /**
     * Supported client types.
     */
    private const ALLOWED_CLIENTS = [self::PHPREDIS_CLIENT, self::PREDIS_CLIENT];
    ...
    /**
     * Get a Redis connection by name, with optional database index.
     *
     * Usage:
     *   Redis::use('cache')        — use the 'cache' connection, default database
     *   Redis::use('cache', 1)     — use the 'cache' connection, database 1
     *   Redis::use('default', 2)   — use the 'default' connection, database 2
     */
    public static function use(string $name = 'default', ?int $database = null): Connection
    {
        if ($database === null) {
            return static::connection($name);
        }

        $connectionName = $name . ':db' . $database;
    ...
    /**
     * Proxy static calls to default connection.
     *
     * @throws Throwable
     */
